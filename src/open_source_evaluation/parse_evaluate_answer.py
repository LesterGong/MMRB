...
def extract_predicted_answer4(cot_answer):
    ...
    match = re.match(r'^([A-H])[\.\):：[]', cleaned)
    if match:
        answer = match.group(1)
        return answer
    lower_cleaned = cleaned.strip().lower()
    if lower_cleaned in ['yes', '1'] or lower_cleaned.startswith("yes["):
        return 'YES'
    elif lower_cleaned in ['no', '0'] or lower_cleaned.startswith("no["):
        return 'NO'

    if len(cleaned) == 0:
        return None

    if re.fullmatch(r'[A-Ha-h]', cleaned):
        return cleaned.upper()

    return cleaned

def extract_predicted_answer3(cot_answer):
    if not isinstance(cot_answer, str):
        cot_answer = str(cot_answer)
    ...
    option_match = re.search(r'\b([A-H])[\.\):：]]', cot_answer)
    if option_match:
        return option_match.group(1).strip().upper()
    option_match = re.search(r'^\s*([A-H])\s*$', cot_answer.strip(), re.IGNORECASE)
    if option_match:
        return option_match.group(1).strip().upper()
    yn_match = re.search(r'\b(yes|no)\b', cot_answer, re.IGNORECASE)
    if yn_match:
        return yn_match.group(1).strip().capitalize()

    possible_lines = [line.strip() for line in cot_answer.strip().split('\n') if line.strip()]
    if possible_lines:
        last_line = possible_lines[-1]
        if len(last_line) <= 30 and not re.search(r'[。．.!\?？]$', last_line):
            return last_line

    return None

def extract_predicted_answer1(cot_answer):
    if not isinstance(cot_answer, str):
        cot_answer = str(cot_answer)
